...
HR1 <- 0.75
HR2 <- 0.75
res0 <- get.figS1.data(S0, L0, td, lam1c, HR1, HR2)
...
res1 <- get.figS1.data(S1, L, td, lam1c, HR1, HR2)

lam1c <- lamc; HR1 <- c(1.3); td <- 3
HR2 <- get.HR2(N, B, td, lam1c, HR1, AHR, pow0, alpha)
S2 <- paste0("NPH(3,1.3,",round(HR2,4),")")
L <- B+find.fu(N, B, td, lam1c, HR1, HR2, pow0, alpha)
res2 <- get.figS1.data(S2, L, td, lam1c, HR1, HR2)

lam1c <- lamc; HR1 <- c(1); td <- 6
HR2 <- get.HR2(N, B, td, lam1c, HR1, AHR, pow0, alpha)
S3 <- paste0("NPH(6,1,",round(HR2,4),")")
L <- B+find.fu(N, B, td, lam1c, HR1, HR2, pow0, alpha)
res3 <- get.figS1.data(S3, L, td, lam1c, HR1, HR2)

plot.figS1(res0, res1, res2, res3, "figS1.pdf")
